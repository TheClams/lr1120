...
//!
//! The LR1120 is a dual band (sub-GHz and 2.4GHz) radio transceiver targetting LoRa and generic (G)-FSK protocols.
//! It also includes a crypto engine and geo-location features (via GNSS or Wifi scanning)
//!
//! ## Features
//!
...
impl CmdBuffer {
    ...
    /// Return the first byte as a status (after reading request)
    pub fn short_status(&self) -> Status {
        self.0[0].into()
    }
    ...
    /// Update the status with the first byte from a response
    ...
    O: OutputPin, SPI: SpiBus<u8>, M: BusyPin
{
    ...
    pub async fn cmd_wr_begin(&mut self, req: &[u8]) -> Result<(), Lr1120Error> {
    ...
    /// Write a command
    pub async fn cmd_wr(&mut self, req: &[u8]) -> Result<(), Lr1120Error> {
    ...
    pub async fn cmd_rd(&mut self, req: &[u8], rsp: &mut [u8]) -> Result<(), Lr1120Error> {
    ...
    pub async fn cmd_data_rw(&mut self, opcode: &[u8], data: &mut [u8]) -> Result<(), Lr1120Error> {
        self.cmd_wr_begin(opcode).await?;
        self.spi
