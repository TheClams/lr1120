...
def gen_enum(field: Field) -> str:
    ...
    if enum_name == 'LoraBw' :
        ...
        lines.append("        matches!(self, Cr5Ham45Li|Cr6Ham23Li|Cr7Ham12Li)")
        lines.append("    }")
    ...
    if enum_name in ['PacketType', 'HwType', 'LoraCr', 'WifiStandard', 'FrameType', 'MacOrigin']:
        ...
        *rest, (first_name, _first_value) = reversed(field.enum.items())
        for variant_name, value in rest:
            name = snake_to_pascal(variant_name)
            lines.append(f"            {value} => {enum_name}::{name},")
        lines.append(f"            _ => {enum_name}::{snake_to_pascal(first_name)},")
        lines.append( "        }")
        lines.append( "    }")
        lines.append( "}")
...
def gen_rsp(cmd: Command, _category: str, advanced: bool = False) -> str:
    ...
    for field in fields:
        ...
        if dim > 1:
            ...
            msb, lsb = pos.get_bit_range_tuple()
            if (field.signed or field.enum) and field.bit_width!=8:
                l+='('
    ...
    if cmd.name == 'GetErrors':
        ...
        lines.append("        u16::from_be_bytes([self.0[1], self.0[2]])")
        lines.append("    }\n")
        lines.append("    /// Flag when no error are present")
        lines.append("    pub fn none(&self) -> bool {")
        lines.append("        self.0[1] == 0 && self.0[2] == 0")
        lines.append("    }")
    ...
    if cmd.name == 'GetVersion':
        ...
        lines.append("        if self.adc_calib()     {defmt::write!(f, \"AdcCalib \")};")
